...
function NeSessions(props) {
  const [NeSessionsConfig, setNeSessionsConfig] = useState(null)
  const [Response, setResponse] = useState("")
  var selectedNeSessions = []

  const tableHeaders = [
    "Name",
    "Type/Version",
    "Interface Running Mode",
    "Association",
    "Technology",
    "Protocol",
    "Location",
    ...
    "License",
    "Connection",
    "Last Communicated / Connected Time",
    "Channels(M/O/B)",
    "Trace",
    "Trace Report",
    "Last Cmd Exec Time Taken(Ms)",
    "Execution Type",
    "Site Info",
    "Ext. Process",
    "Remarks",
                ...
                {NeSessionsConfig.map((data) => {
                  return (
                    <tr key={data.strName} className="bg-white border-b-2">
                      ...
                      <td className="text-black px-6 py-4">{data.dwNeType}</td>
                      <td className="text-black px-6 py-4">
                        {data.strRemoteInterfaceRunningMode}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.dwAssociationType}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.strNeTechnology}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.strNeProtocol}
                      </td>
                      ...
                      <td className="text-black px-6 py-4">
                        {data.bLicensed ? "True" : "False"}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.bConnected ? "True" : "False"}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.dwLastCommunicatedTimestampMs}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.dwMaxConcurrentConnections /
                          data.dwOpenedConnectionsCount /
                          data.dwBusyConnectionsCount}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.dwTraceLevel}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.bTraceReportOn ? "True" : "False"}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.dwLastCommandExecTimeTakenMSec}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.dwNeExecutionType}
                      </td>
                      <td className="text-black px-6 py-4">
                        {data.dwSiteType}
                      </td>
  ...
  function getNeSessions() {
    // URL to connect to server
    const URL = "http://" + props.host + ":" + props.port

    //get method with headers
    get(URL + "/GetNetworkElementsStatus")
      .query({ dwGroupingCategory: 0, strName: "" })
      .set({
